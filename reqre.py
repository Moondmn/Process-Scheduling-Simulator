...
while True:
    try:
        ...
        filename = (
            driver.find_element(by="class name", value="soundTitle__title")
            .find_element(by="tag name", value="span")
            .text
        )
        break
    except:
        print(
            "Chromedriver could not get the correct info, Selenium needs to reload the page"
        )
        driver.refresh()
...
subprocess.call(
    'ffmpeg -protocol_whitelist file,http,https,tcp,tls -i "{}" -b:a 320k "file.mp3"'.format(
        link
    ),
...
with open("embed1.log", "w") as log:
    ext = subprocess.call(
        'ffmpeg -i file.mp3 -protocol_whitelist file,http,https,tcp,tls -i "{}" -map 0:a -map 1:0 -c:a copy -id3v2_version 3 "file2.mp3"'.format(
            art
        ),
        stdout=log,
        stderr=log,
...
print("Embedding Complete!") if ext == 0 else print(
    "Embedding Failed! Using original file..."
)
...
if ext == 1:
    ...
    with open("embed2.log", "w") as log:
        ext = subprocess.call(
            'ffmpeg -i file.mp3 -i art.jpg -map 0:a -map 1:0 -c:a copy -id3v2_version 3 "file2.mp3"'.format(
                art
            ),
            stdout=log,
            stderr=log,
...
target = filedialog.asksaveasfilename(
    ...
    initialfile="{}.mp3".format(filename),
    title="Save as...",
